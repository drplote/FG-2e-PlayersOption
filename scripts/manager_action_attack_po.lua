...
function onAttackOverride(rSource, rTarget, rRoll)
    ...
    local nDefenseVal, nAtkEffectsBonus, nDefEffectsBonus, nACShield =
        ActorManagerPO.getDefenseValue(rSource, rTarget, rRoll);
    ...
    if rTarget and bHitTarget and not bPsionic then
        ...
        elseif nMirrorCount > 0 then
            ...
            local rMirrorRoll = {
                sType = "roll_against_mirrorimages",
                sDesc = "[MIRROR-IMAGE]",
                aDice = aMirrorDice,
                nMod = 0,
                bSecret = false,
...
function applyAlternateEffectModifiers(rTarget, rRoll)
    ...
    if not bIsProneOrUnconscious and not bIsStunnedOrRestrained and
        EffectManager5E.hasEffectCondition(rTarget, "Paralyzed") then
