...
function getConstitution(rChar)
    ...
    local nCon = DB.getValue(nodeActor, "abilities.constitution.total",
        DB.getValue(nodeActor, "abilities.constitution.score", 0));
    return nCon;
end
    ...
    if bPsionic then -- calculate defenses for psionics
        ...
        if rRoll.Psionic_DisciplineType == "attack" then
            ...
            if (nodeSpell) then
                ...
                if sSpellName ~= "" then
                    nAttackEffectMod = ActorManagerADND.getPsionicAttackVersusDefenseMode(rDefender, rAttacker,
                        sSpellName, nAttackEffectMod);
                end
            end
        end
        ...
        if sDefenderType == "pc" or PlayerOptionManager.shouldUseDynamicNpcAc() then
            ...
            -- or if you are prone
            if bNoShield or bRearAtk or bUnconscious or bParalyzed or bRestrainedStunned or
                (bProne and not PlayerOptionManager.isUsingAlternateTargetEffectModifiers()) or
        ...
        if sDefenderType == "pc" or PlayerOptionManager.shouldUseDynamicNpcAc() then
            ...
            if bNoDex or bRearAtk or bProne or bParalyzed or bRestrainedStunned or
                ActionSave.hasConcentrationEffects(rDefender) or EffectManager5E.hasEffect(rDefender, "BLINDED", nil) or
        ...
        if ActorManager.hasCT(rDefender) then
            ...
            local aBonusTargetedAttackDice, nBonusTargetedAttack =
                EffectManager5E.getEffectsBonus(rAttacker, "ATK", false, aAttackFilter, rDefender, true);
            nAttackEffectMod = nAttackEffectMod + StringManager.evalDice(aBonusTargetedAttackDice, nBonusTargetedAttack);

            local aACEffects, nACEffectCount = EffectManager5E.getEffectsBonusByType(rDefender, {"AC"}, true,
                aAttackFilter, rAttacker);
