...
function canDamageTypeHurtArmor(aDmgTypes, nodeArmor)
    ...
    elseif not aDmgTypes or #aDmgTypes == 0 then
        ...
        if PlayerOptionManager.isMagicArmorDamagedByEqualMagic() then
            if nBonus <= 6 then
                table.insert(aDamagingTypes, "magic +6");
            end
            if nBonus <= 5 then
                table.insert(aDamagingTypes, "magic +5");
            end
            if nBonus <= 4 then
                table.insert(aDamagingTypes, "magic +4");
            end
            if nBonus <= 3 then
                table.insert(aDamagingTypes, "magic +3");
            end
            if nBonus <= 2 then
                table.insert(aDamagingTypes, "magic +2");
            end
            if nBonus <= 1 then
                table.insert(aDamagingTypes, "magic +1");
                table.insert(aDamagingTypes, "magic");
            end
        end
