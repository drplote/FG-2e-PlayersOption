...
function onCritCommand(sCmd, sParams)
    ...
    if #aParams == 3 then
        ...
        if (sType == "p" or sType == "b" or sType == "s") and (nSeverity < 25 and nSeverity > 0) and
            (nLocation > 0 and nLocation < 10001) then
            bValidParams = true;
        end
    end
    if not bValidParams then
        ChatManagerPO.deliverChatMessage(
            "Usage: /hmcrit [damage type (s,p, or b)] [#severity (1-24)] [#location (1-10000)]");
    else
...
function getCritResult(nSeverity, rHitLocation, aDamageTypes, rSource, rTarget)
    ...
    sResult = sResult .. "\r[Location (d10000=" .. rHitLocation.roll .. "): " .. rHitLocation.desc .. "(" ..
                  rHitLocation.side .. ")]";
    if nSeverity <= 0 then
        ...
        sResult = sResult .. "\r[Bruised: " .. nBruiseDays ..
                      " days. If injured in same location again before healed, suffer +1 damage per injury.]";
        if nSeverity > 5 then
...
function getCritSeverity(rRoll, rAction, nDefenseVal, rSource, rTarget)
    ...
    local nFinalSeverity = math.min(24, nBaseSeverity + nSeverityDieRoll +
        CritManagerPO.getCritSeverityBonus(rSource, rTarget, rAction));
...
function decodeMinorConcussion(aEffects, nSeverity)
    ...
    table.insert(aEffects,
        "Gains " .. sFlaws .. " with an immediate headache. Lasts " .. nDuration .. " hours or until healed.");
end
...
function createProfuseBleedingMessage(rTarget)
    local nConstitution = ActorManagerPO.getConstitution(rTarget);
    local nHalfCon = math.max(1, math.floor(nConstitution / 2));
    return "Profuse bleeding! Will bleed to death in " .. nHalfCon ..
               " rounds unless the wound has been treated by a successful first aid-related skill check or any cure spell that heals half the wound's HPs in damage, or one Cure Critical Wounds or better spell. Severed limbs may be cauterized by applying open flame for one round (1d4 damage)";
end
...
function decodeMuscleTear(aEffects, nIndex, nSeverity, rLocation, rTarget)
    ...
    table.insert(aEffects, "Muscle Tear (" .. getMuscle(rLocation, nIndex) ..
        ")! These wounds heal naturally at half normal rate. Any Dexterity and Strength reductions from this crit last for " ..
        nDays ..
        " days, then are reduced by half for like periods until reduce to zero. This lasting effect occurs regardless of whether the wounds have been healed fully by spells. A Cure Critical Wounds spell or better will eliminate all ill effects instantly.");
...
function decodeBrokenBone(aEffects, nFracture, nIndex, nSeverity, rLocation, rTarget)
    ...
    table.insert(aEffects, sEffectName .. " Can be cured by magical means or through healing at " .. sHealRate ..
        "  the normal rate. Successfully setting a broken bone using first aid-related skills allows healing at one quarter the normal rate. Unless set properly prior to healing, even magical healing, fractures will heal incorrectly giving rise to lasting limps, obvious lumps, etc. In this case, half of any associated movement and/or ability score penalties will be permanent. A Cure Critical Wounds spell or better will eliminate all ill effects instantly.");
...
function decodeTornLigaments(aEffects, nIndex, nSeverity, rLocation, rTarget)
    ...
    table.insert("Torn ligament or tendon (" .. getMuscle(rLocation, nIndex) ..
                     ")! Unless the appropriate body part is isolated prior to healing, even magical healing, will heal incorrectly or incompletely, giving rise to lasting limps, obvious lumps, etc. In this case, half of any associated movement and/or ability score penalties will become permanent. A Cure Critical Wounds spell or better will eliminate all ill effects instantly.");
...
function decodeInternalBleeding(aEffects)
    table.insert(aEffects,
        "Internal bleeding! Each hour lose 1d4 hit points and make a Constitution check, with failure indicating that the character goes into shock (see Trauma Damage). May live for many hours or days with this problem and not know it; will feel pains in the area, but will otherwise not know that he has been injured");
end

function decodeUnconscious(aEffects)
    table.insert(aEffects,
        "Falls to the ground unconscious. Remains in a coma until the hit points suffered from this wound are healed (naturally or magically)");
end

function decodeLimbSevered(aEffects, rLocation, rTarget)
    table.insert(aEffects,
        "Limb severed! The stump can be cured by magical means or through natural healing at one third the normal rate. Regeneration, Reattach Limb, or the like needed to recover the limb.");
...
function rollVitalOrganDamage(rLocation, nIndex, nRollValue)
    ...
    local bWasHit = true;
    local sMsg = "";
    if nRollValue < 26 then
        sMsg = "Vital organ missed!";
        bWasHit = false;
    elseif nRollValue < 51 then
        sMsg = "Vital organ (" .. getOrgan(rLocation, nIndex) .. ")! Lose " .. DiceManagerPO.getDiceResult(2, 6) ..
                   " points of " .. sStatLost .. ". 1 point returns per day for the next " .. math.random(1, 6) ..
                   " day(s). Unreturned points are lost permanently.";
    elseif nRollValue < 71 then
        sMsg = "Vital organ (" .. getOrgan(rLocation, nIndex) .. ")! Death in " .. math.random(1, 12) .. " days";
    elseif nRollValue < 81 then
        sMsg = "Vital organ (" .. getOrgan(rLocation, nIndex) .. ")! Death in " .. math.random(1, 12) .. " hours";
    elseif nRollValue < 91 then
        sMsg = "Vital organ (" .. getOrgan(rLocation, nIndex) .. ")! Death in " .. math.random(1, 12) .. " rounds";
    else
        sMsg = "Vital organ(" .. getOrgan(rLocation, nIndex) .. ")! Death in " .. math.random(1, 12) .. " segments";
    end
    return sMsg, bWasHit;
end
...
function decodeMovement(aEffects, nValue)
    local sMsg = "";
    if nValue == 1 then
        sMsg = "Lose 50% move for 1 rd, then 10% for " .. DiceManagerPO.getDiceResult(2, 4) .. " rds";
    elseif nValue == 2 then
        sMsg = "Lose 50% move for 2 rds, then 25% for " .. DiceManagerPO.getDiceResult(2, 10) .. " rds";
    elseif nValue == 3 then
        sMsg = "Lose 50% move for 1 rd, 10% for " .. DiceManagerPO.getDiceResult(2, 4) .. " rds, then 25% for " ..
                   DiceManagerPO.getDiceResult(1, 4) .. " turns";
    elseif nValue == 4 then
        sMsg = "Lose 50% move for " .. DiceManagerPO.getDiceResult(1, 12) .. " hours";
    elseif nValue == 5 then
        sMsg = "Lose 50% move for " .. DiceManagerPO.getDiceResult(1, 12) .. " hours, then 25% for " ..
                   DiceManagerPO.getDiceResult(1, 12) .. " days";
    elseif nValue == 6 then
        sMsg = "Lose 75% move for 6 hours, then 50% for " .. DiceManagerPO.getDiceResult(2, 12) .. " days";
    elseif nValue == 7 then
        sMsg = "Lose 75% move for 6 hours, then 50% for " .. DiceManagerPO.getDiceResult(4, 12) .. " days";
    elseif nValue == 8 then
        sMsg = "Lose 75% move for 6 hours, then 50% for " .. DiceManagerPO.getDiceResult(1, 3) .. " months";
    elseif nValue == 9 then
        sMsg = "Lose 75% move for 1 day, then 50% for " .. DiceManagerPO.getDiceResult(1, 4) .. " months";
    else
        sMsg = "Lose 75% move for 1 week, then 50% for " .. DiceManagerPO.getDiceResult(1, 6) .. " months";
    end
    table.insert(aEffects, sMsg);
end
...
function decodeMaxDamageBonus(rLocation, rTarget)
    if rLocation.dam == 1.0 then
        ...
        if rLocation.isDigit then
            return "If bonus damage exceeds " .. rLocation.dam * 100 .. "% of health (" .. nAffectedHp ..
                       "), affected body part is severed or destroyed.";
        else
            -- TODO: Once we calculate if it was severed, this could cause profuse bleeding
            return "If bonus damage exceeds " .. rLocation.dam * 100 .. "% of health (" .. nAffectedHp ..
                       "), affected body part is severed or destroyed. If severed, will cause " ..
                       createProfuseBleedingMessage(rTarget);
        end
    end
end
...
function buildHackingCritMatrix()
    ...
    aHackingCritMatrix["Foot, top"][3] = {
        db = 3
    };
    aHackingCritMatrix["Foot, top"][4] = {
        db = 3
    };
    aHackingCritMatrix["Foot, top"][5] = {
        db = 4,
        m = 1
    };
    aHackingCritMatrix["Foot, top"][6] = {
        db = 4,
        m = 1
    };
    aHackingCritMatrix["Foot, top"][7] = {
        db = 6,
        t = 1,
        m = 2
    };
    aHackingCritMatrix["Foot, top"][8] = {
        db = 6,
        t = 1,
        m = 2
    };
    aHackingCritMatrix["Foot, top"][9] = {
        db = 8,
        t = 1,
        m = 3
    };
    aHackingCritMatrix["Foot, top"][10] = {
        db = 8,
        t = 1,
        m = 3
    };
    aHackingCritMatrix["Foot, top"][11] = {
        dm = 2,
        t = 1,
        m = 4
    };
    aHackingCritMatrix["Foot, top"][12] = {
        dm = 2,
        t = 1,
        m = 4
    };
    aHackingCritMatrix["Foot, top"][13] = {
        dm = 2,
        t = 1,
        m = 5
    };
    aHackingCritMatrix["Foot, top"][14] = {
        dm = 2,
        t = 1,
        m = 5
    };
    aHackingCritMatrix["Foot, top"][15] = {
        dm = 2,
        t = 1,
        m = 6
    };
    aHackingCritMatrix["Foot, top"][16] = {
        dm = 2,
        t = 1,
        m = 6
    };
    aHackingCritMatrix["Foot, top"][17] = {
        dm = 2,
        t = 1,
        m = 7
    };
    aHackingCritMatrix["Foot, top"][18] = {
        dm = 2,
        t = 1,
        m = 7
    };
    aHackingCritMatrix["Foot, top"][19] = {
        dm = 2,
        t = 1,
        m = 7,
        b = 1
    };
    aHackingCritMatrix["Foot, top"][20] = {
        dm = 2,
        t = 1,
        m = 8,
        bm = 1
    };
    aHackingCritMatrix["Foot, top"][21] = {
        dm = 2,
        t = 1,
        bf = 1,
        m = 8
    };
    aHackingCritMatrix["Foot, top"][22] = {
        dm = 2,
        t = 1,
        bf = 1,
        m = 9
    };
    aHackingCritMatrix["Foot, top"][23] = {
        dm = 2,
        ls = true,
        m = 9
    };
    aHackingCritMatrix["Foot, top"][24] = {
        dm = 2,
        ls = true,
        m = 10
    };
    ...
    aHackingCritMatrix["Heel"][1] = {
        db = 1
    };
    aHackingCritMatrix["Heel"][2] = {
        db = 1
    };
    aHackingCritMatrix["Heel"][3] = {
        db = 3
    };
    aHackingCritMatrix["Heel"][4] = {
        db = 3
    };
    aHackingCritMatrix["Heel"][5] = {
        db = 4,
        m = 1
    };
    aHackingCritMatrix["Heel"][6] = {
        db = 4,
        m = 1
    };
    aHackingCritMatrix["Heel"][7] = {
        db = 6,
        t = 1,
        m = 2
    };
    aHackingCritMatrix["Heel"][8] = {
        db = 6,
        t = 1,
        m = 2,
        a = 1
    };
    aHackingCritMatrix["Heel"][9] = {
        db = 8,
        t = 1,
        m = 3,
        a = 1
    };
    aHackingCritMatrix["Heel"][10] = {
        db = 8,
        t = 1,
        m = 3,
        a = 2
    };
    aHackingCritMatrix["Heel"][11] = {
        dm = 2,
        t = 1,
        m = 4,
        a = 2
    };
    aHackingCritMatrix["Heel"][12] = {
        dm = 2,
        t = 1,
        d = 1,
        m = 4
    };
    aHackingCritMatrix["Heel"][13] = {
        dm = 2,
        t = 1,
        a = 2,
        d = 2,
    ...
    aHackingCritMatrix["Toe(s)"][1] = {
        db = 1
    };
    aHackingCritMatrix["Toe(s)"][2] = {
        db = 1
    };
    aHackingCritMatrix["Toe(s)"][3] = {
        db = 3
    };
    aHackingCritMatrix["Toe(s)"][4] = {
        db = 3
    };
    aHackingCritMatrix["Toe(s)"][5] = {
        db = 4,
        m = 1
    };
    aHackingCritMatrix["Toe(s)"][6] = {
        db = 4,
        m = 1
    };
    aHackingCritMatrix["Toe(s)"][7] = {
        db = 6,
        t = 1,
        m = 2
    };
    aHackingCritMatrix["Toe(s)"][8] = {
        db = 6,
        t = 1,
        m = 2
    };
    aHackingCritMatrix["Toe(s)"][9] = {
        db = 8,
        t = 1,
        m = 3
    };
    aHackingCritMatrix["Toe(s)"][10] = {
        db = 8,
        t = 1,
        m = 3
    };
    aHackingCritMatrix["Toe(s)"][11] = {
        dm = 2,
        t = 1,
        m = 4
    };
    aHackingCritMatrix["Toe(s)"][12] = {
        dm = 2,
        t = 1,
        m = 4
    };
    aHackingCritMatrix["Toe(s)"][13] = {
        dm = 2,
        t = 1,
        m = 5
    };
    aHackingCritMatrix["Toe(s)"][14] = {
        dm = 2,
        t = 1,
        m = 5
    };
    aHackingCritMatrix["Toe(s)"][15] = {
        dm = 2,
        t = 1,
        m = 6
    };
    aHackingCritMatrix["Toe(s)"][16] = {
        dm = 2,
        t = 1,
        b = 1,
        m = 6
    };
    aHackingCritMatrix["Toe(s)"][17] = {
        dm = 2,
        t = 1,
        bm = 1,
        m = 7
    };
    aHackingCritMatrix["Toe(s)"][18] = {
        dm = 2,
        t = 1,
        bm = 1,
        m = 7
    };
    aHackingCritMatrix["Toe(s)"][19] = {
        dm = 2,
        ls = true,
        m = 7
    };
    aHackingCritMatrix["Toe(s)"][20] = {
        dm = 2,
        ls = true,
        m = 8
    };
    aHackingCritMatrix["Toe(s)"][21] = {
        dm = 2,
        ls = true,
        m = 8
    };
    aHackingCritMatrix["Toe(s)"][22] = {
        dm = 2,
        ls = true,
        m = 9
    };
    aHackingCritMatrix["Toe(s)"][23] = {
        dm = 2,
        ls = true,
        m = 9
    };
    aHackingCritMatrix["Toe(s)"][24] = {
        dm = 2,
        ls = true,
        m = 10
    };
    ...
    aHackingCritMatrix["Foot, arch"][1] = {
        db = 1
    };
    aHackingCritMatrix["Foot, arch"][2] = {
        db = 1
    };
    aHackingCritMatrix["Foot, arch"][3] = {
        db = 3
    };
    aHackingCritMatrix["Foot, arch"][4] = {
        db = 3
    };
    aHackingCritMatrix["Foot, arch"][5] = {
        db = 4,
        m = 1
    };
    aHackingCritMatrix["Foot, arch"][6] = {
        db = 4,
        m = 1
    };
    aHackingCritMatrix["Foot, arch"][7] = {
        db = 6,
        t = 1,
        m = 2
    };
    aHackingCritMatrix["Foot, arch"][8] = {
        db = 6,
        t = 1,
        m = 2,
        a = 1
    };
    aHackingCritMatrix["Foot, arch"][9] = {
        db = 8,
        t = 1,
        m = 3,
        a = 1
    };
    aHackingCritMatrix["Foot, arch"][10] = {
        db = 8,
        t = 1,
        m = 3,
        a = 2
    };
    aHackingCritMatrix["Foot, arch"][11] = {
        dm = 2,
        t = 1,
        m = 4,
        a = 2
    };
    aHackingCritMatrix["Foot, arch"][12] = {
        dm = 2,
        t = 1,
        a = 2,
        d = 1,
    ...
    aHackingCritMatrix["Foot, arch"][13] = {
        dm = 2,
        t = 1,
        a = 2,
        d = 2,
    ...
    aHackingCritMatrix["Foot, arch"][14] = {
        dm = 2,
        b = 1,
        a = 2,
        d = 2,
    ...
    aHackingCritMatrix["Foot, arch"][15] = {
        dm = 2,
        b = 1,
        a = 2,
        d = 2,
    ...
    aHackingCritMatrix["Foot, arch"][16] = {
        dm = 2,
        b = 1,
        a = 3,
        d = 2,
    ...
    aHackingCritMatrix["Foot, arch"][17] = {
        dm = 2,
        b = 1,
        a = 3,
        d = 2,
    ...
    aHackingCritMatrix["Foot, arch"][18] = {
        dm = 2,
        b = 1,
        a = 3,
        d = 2,
    ...
    aHackingCritMatrix["Foot, arch"][19] = {
        dm = 2,
        bm = 1,
        a = 3,
        d = 2,
    ...
    aHackingCritMatrix["Foot, arch"][20] = {
        dm = 2,
        bm = 1,
        a = 3,
        d = 2,
    ...
    aHackingCritMatrix["Foot, arch"][21] = {
        dm = 2,
        bf = 1,
        a = 3,
        d = 2,
    ...
    aHackingCritMatrix["Foot, arch"][22] = {
        dm = 2,
        t = 1,
        bf = 1,
        a = 3,
    ...
    aHackingCritMatrix["Foot, arch"][23] = {
        dm = 2,
        ls = true,
        a = 3,
        d = 2,
    ...
    aHackingCritMatrix["Foot, arch"][24] = {
        dm = 2,
        ls = true,
        a = 3,
        d = 2,
    ...
    aHackingCritMatrix["Neck, front"][1] = {
        db = 3
    };
    aHackingCritMatrix["Neck, front"][2] = {
        db = 4
    };
    aHackingCritMatrix["Neck, front"][3] = {
        db = 6
    };
    aHackingCritMatrix["Neck, front"][4] = {
        db = 8
    };
    aHackingCritMatrix["Neck, front"][5] = {
        dm = 2
    };
    aHackingCritMatrix["Neck, front"][6] = {
        dm = 2,
        ws = true,
        a = 1,
        d = 1
    };
    aHackingCritMatrix["Neck, front"][7] = {
        dm = 2,
        w = true,
        a = 1,
        d = 1
    };
    aHackingCritMatrix["Neck, front"][8] = {
        dm = 2,
        f = true,
        a = 1,
        d = 1
    };
    aHackingCritMatrix["Neck, front"][9] = {
        dm = 2,
        a = 2,
        d = 2,
        f = true
    };
    aHackingCritMatrix["Neck, front"][10] = {
        dm = 2,
        a = 2,
        d = 2,
        f = true,
    ...
    aHackingCritMatrix["Neck, front"][11] = {
        dm = 2,
        a = 2,
        d = 2,
        f = true,
    ...
    aHackingCritMatrix["Neck, front"][12] = {
        dm = 3,
        a = 3,
        d = 3,
        u = true
    };
    aHackingCritMatrix["Neck, front"][13] = {
        dm = 3,
        a = 3,
        d = 3,
        u = true
    };
    aHackingCritMatrix["Neck, front"][14] = {
        dm = 3,
        a = 3,
        d = 3,
        u = true,
    ...
    aHackingCritMatrix["Neck, front"][15] = {
        dm = 3,
        a = 3,
        d = 4,
        u = true,
    ...
    aHackingCritMatrix["Neck, front"][16] = {
        dm = 3,
        a = 3,
        d = 5,
        u = true,
    ...
    aHackingCritMatrix["Neck, front"][17] = {
        dm = 4,
        a = 4,
        d = 6,
        u = true,
    ...
    aHackingCritMatrix["Neck, front"][18] = {
        dm = 4,
        a = 5,
        p = true,
        u = true,
    ...
    aHackingCritMatrix["Neck, front"][19] = {
        dm = 4,
        pb = true,
        ib = true,
        v = 1
    };
    aHackingCritMatrix["Neck, front"][20] = {
        dm = 4,
        pb = true,
        ib = true,
        v = 1
    };
    aHackingCritMatrix["Neck, front"][21] = {
        dm = 4,
        p = true,
        u = true,
        sc = true
    };
    aHackingCritMatrix["Neck, front"][22] = {
        dm = 4,
        pb = true,
        ib = true,
        v = 2
    };
    aHackingCritMatrix["Neck, front"][23] = {
        dead = "decapitated"
    };
    aHackingCritMatrix["Neck, front"][24] = {
        dead = "decapitated"
    };
...
function buildCrushingCritMatrix()
    ...
    aCrushingCritMatrix["Neck, front"][1] = {
        db = 3
    };
    aCrushingCritMatrix["Neck, front"][2] = {
        db = 4
    };
    aCrushingCritMatrix["Neck, front"][3] = {
        db = 6
    };
    aCrushingCritMatrix["Neck, front"][4] = {
        db = 8
    };
    aCrushingCritMatrix["Neck, front"][5] = {
        dm = 2
    };
    aCrushingCritMatrix["Neck, front"][6] = {
        dm = 2,
        ws = true,
        a = 1,
        d = 1
    };
    aCrushingCritMatrix["Neck, front"][7] = {
        dm = 2,
        w = true,
        a = 1,
        d = 1
    };
    aCrushingCritMatrix["Neck, front"][8] = {
        dm = 2,
        f = true,
        a = 1,
        d = 1
    };
    aCrushingCritMatrix["Neck, front"][9] = {
        dm = 2,
        a = 2,
        d = 2,
        f = true
    };
    aCrushingCritMatrix["Neck, front"][10] = {
        dm = 2,
        a = 2,
        d = 2,
        f = true,
    ...
    aCrushingCritMatrix["Neck, front"][11] = {
        dm = 2,
        a = 2,
        d = 2,
        f = true,
    ...
    aCrushingCritMatrix["Neck, front"][12] = {
        dm = 3,
        a = 3,
        d = 3,
        u = true
    };
    aCrushingCritMatrix["Neck, front"][13] = {
        dm = 3,
        a = 3,
        d = 3,
        u = true
    };
    aCrushingCritMatrix["Neck, front"][14] = {
        dm = 3,
        a = 3,
        d = 3,
        u = true,
    ...
    aCrushingCritMatrix["Neck, front"][15] = {
        dm = 3,
        a = 3,
        d = 3,
        u = true,
    ...
    aCrushingCritMatrix["Neck, front"][16] = {
        dm = 3,
        a = 3,
        d = 4,
        u = true,
    ...
    aCrushingCritMatrix["Neck, front"][17] = {
        dm = 4,
        a = 4,
        d = 5,
        b = 1,
    ...
    aCrushingCritMatrix["Neck, front"][18] = {
        dm = 4,
        a = 5,
        d = 6,
        b = 1,
    ...
    aCrushingCritMatrix["Neck, front"][19] = {
        dm = 4,
        a = 5,
        d = 6,
        b = 1,
    ...
    aCrushingCritMatrix["Neck, front"][20] = {
        cm = 4,
        a = 5,
        d = 6,
        b = 2,
    ...
    aCrushingCritMatrix["Neck, front"][21] = {
        dm = 4,
        p = true,
        u = true,
        sc = true
    };
    aCrushingCritMatrix["Neck, front"][22] = {
        dead = "windpipe crushed"
    };
    aCrushingCritMatrix["Neck, front"][23] = {
        dead = "neck snapped"
    };
    aCrushingCritMatrix["Neck, front"][24] = {
        dead = "neck snapped"
    };
...
function buildCritLocationInfo()
    aCritLocations["Foot, top"] = {
        dam = 0.1,
        isLeg = true,
        skeletal = {"Tarsus"},
        muscular = {"1d5 Extensor Tendons of the Toes"}
    };
    aCritLocations["Heel"] = {
        dam = 0.1,
        isLeg = true,
        skeletal = {"Os Calcis"},
        muscular = {"Achilles Tendon"}
    };
    aCritLocations["Toe(s)"] = {
        dam = 0.01,
        isLeg = true,
        isDigit = true,
        skeletal = {"1d10 Metatarsus and Phalanges Bones"},
        muscular = {"1d5 Extensor Tendons of the Toes"}
    };
    aCritLocations["Foot, arch"] = {
        dam = 0.1,
        isLeg = true,
        skeletal = {"Tarsus"},
        muscular = {"Extensor Brevis Hallucis"}
    };
    aCritLocations["Ankle, inner"] = {
        dam = 0.15,
        isLeg = true,
        skeletal = {"Internal Malleolus"},
        muscular = {"Soleus"}
    };
    aCritLocations["Ankle, outer"] = {
        dam = 0.15,
        isLeg = true,
        skeletal = {"External Malleolus"},
        muscular = {"Extensor Longus Digitorum", "Peroneous Longus et Brevis"}
    };
    aCritLocations["Ankle, upper/Achilles"] = {
        dam = 0.15,
        isLeg = true,
        skeletal = {"Internal Malleolus", "External Malleolus"},
        muscular = {"Tibialis Anticus", "Extensor Longus Hallucis"}
    };
    aCritLocations["Shin"] = {
        dam = 0.25,
        isLeg = true,
        skeletal = {"Fibula", "Tibia"},
        muscular = {"Tibialis Anticus", "Soleus"}
    };
    aCritLocations["Calf"] = {
        dam = 0.25,
        isLeg = true,
        skeletal = {"Fibula", "Tibia"},
        muscular = {"Soleus", "Gastrocnemius"}
    };
    aCritLocations["Knee"] = {
        dam = 0.25,
        isLeg = true,
        skeletal = {"Outer Tuberosity of the Femur", "Patella", "Inner Tuberosity of the Femur"},
        muscular = {"Patella Tendon"}
    };
    aCritLocations["Knee, back"] = {
        dam = 0.25,
        isLeg = true,
        skeletal = {"Condyle of Femur"},
        muscular = {"Gastrocnemius"}
    };
    aCritLocations["Hamstring"] = {
        dam = 1.0,
        isLeg = true,
        skeletal = {"Femur"},
        muscular = {"Biceps Femoris"}
    };
    aCritLocations["Thigh"] = {
        dam = 1.0,
        isLeg = true,
        skeletal = {"Femur"},
        muscular = {"Tendon of the Extensors of the Leg", "Vastus Internus", "Vastus Externus", "Adductor Magnus",
                    "Rectus Femoris"}
    };
    aCritLocations["Hip"] = {
        dam = 1.0,
        isLeg = true,
        skeletal = {"Trochanter", "Neck of the Femur", "Head of the Femur", "Illium"},
        muscular = {"Gluteus Medius", "Tensor Vaginae Femoris"},
        vital = {"Large Intestine", "Small Intestine"}
    };
    aCritLocations["Groin"] = {
        dam = 0.2,
        isLeg = true,
        skeletal = {"Os Pubis"},
        muscular = {"Adductor Magnus", "Extensor Longus Digitorum", "Pectineus", "Adductor Longus"},
        vital = {"Bladder", "Small Intestine"}
    };
    aCritLocations["Buttock"] = {
        dam = 1.0,
        isLeg = true,
        skeletal = {"Ilium", "Sacrum", "Coccyx"},
        muscular = {"Gluteus Maximus"},
        vital = {}
    };
    aCritLocations["Abdomen, Lower"] = {
        dam = 1.0,
        isTorso = true,
        skeletal = {"Ilium", "Os Pubis", "Sacrum"},
        muscular = {"Aponeurosis of Obliquus Externus Abdominis", "Rectus Abdominis"},
        vital = {"Pancreas", "Duodenum", "Kidney", "Small Intestine"}
    };
    aCritLocations["Side, lower"] = {
        dam = 0.8,
        isTorso = true,
        skeletal = {"Crest of the Ilium", "1d2 Lumbar Vertebrae: 2nd to 3rd"},
        muscular = {"Crest of the Ilium", "Obliquus Internus Abdominis"},
        vital = {"Spleen or Liver", "Lung"}
    };
    aCritLocations["Abdomen, upper"] = {
        dam = 1.0,
        isTorso = true,
        skeletal = {"1d2 Lumbar Vertebrae: 2nd to 3rd"},
        muscular = {"Aponeurosis of Obliquus Externus Abdominis", "Rectus Abdominis"},
        vital = {"Spleen", "Stomach", "Liver"}
    };
    aCritLocations["Back, small of"] = {
        dam = 1.0,
        isTorso = true,
        isSpine = true,
        skeletal = {"1d3 Lumbar Vertebrae: 1st to 3rd"},
        muscular = {"External Oblique", "Erector Spinae"},
        vital = {"Kidney", "Spinal Cord"}
    };
    aCritLocations["Back, lower"] = {
        dam = 1.0,
        isTorso = true,
        isSpine = true,
        skeletal = {"1d3 Lumbar Vertebrae: 1st to 3rd"},
        muscular = {"Latissimus Dorsi"},
        vital = {"1d2 Kidneys", "Spinal Cord"}
    };
    aCritLocations["Chest"] = {
        dam = 1.0,
        isTorso = true,
        isSpine = true,
        skeletal = {"1d6 Lower Ribs", "1d6 Upper Ribs", "Sternum: Manubrium", "Sternum: Gladiolus"},
        muscular = {"Aponeurosis of Obliquus Externus Abdominis", "Obliquus Externus Abdominis", "Rectus Abdominis"},
        vital = {"Lung", "Heart", "Spinal Cord"}
    };
    aCritLocations["Side, upper"] = {
        dam = 0.8,
        isTorso = true,
        skeletal = {"1d6 Lower Ribs", "1d6 Upper Ribs"},
        muscular = {"Serratus Magnus", "Obliquus Externus Abdominis"},
        vital = {"Lung"}
    };
    aCritLocations["Back, upper"] = {
        dam = 1.0,
        isTorso = true,
        isSpine = true,
        skeletal = {"Scapula", "1d4 Upper Ribs", "1d4 Middle Ribs", "1d4 Lower Ribs"},
        muscular = {"Deltoid", "Teres Major"},
        vital = {"Spinal Cord"}
    };
    aCritLocations["Back, upper middle"] = {
        dam = 1.0,
        isTorso = true,
        isSpine = true,
        skeletal = {"1d4 Lower Dorsal Vertebrae: 9th to 12th", "1d4 Middle Dorsal Vertebrae: 5th to 8th",
                    "1d4 Upper Dorsal Vertebrae: 1st to 4th"},
        muscular = {"Trapezius"},
        vital = {"Spinal Cord"}
    };
    aCritLocations["Armpit"] = {
        dam = 0.3,
        isArm = true,
        skeletal = {"Head of the Humerus", "Scapula", "1d3 Ribs"},
        muscular = {"Pectoralis Major", "Coraco-brachialis", "Deltoid"},
        vital = {}
    };
    aCritLocations["Arm, upper outer"] = {
        dam = 0.3,
        isArm = true,
        skeletal = {"Humerus"},
        muscular = {"Triceps Extensor Cubiti", "Biceps Flexor Cubiti"},
        vital = {}
    };
    aCritLocations["Arm, upper inner"] = {
        dam = 0.3,
        isArm = true,
        skeletal = {"Humerus"},
        muscular = {"Trcipes Extensor Cubiti", "Biceps Flexor Cubiti"},
        vital = {}
    };
    aCritLocations["Elbow"] = {
        dam = 0.25,
        isArm = true,
        skeletal = {"Olecranon Process of the Ulna"},
        muscular = {"Flexor Longus Pollicis"},
        vital = {}
    };
    aCritLocations["Inner joint"] = {
        dam = 0.25,
        isArm = true,
        skeletal = {"Trochlea", "Radial Head of the Humerus"},
        muscular = {"Pronator Radii Teres", "Supinator Longus"},
        vital = {}
    };
    aCritLocations["Forearm, back"] = {
        dam = 0.25,
        isArm = true,
        skeletal = {"Radius", "Ulna"},
        muscular = {"Extensor Carpi Radialis Brevior", "Extensor Carpi Radialis Longior"},
        vital = {}
    };
    aCritLocations["Forearm, inner"] = {
        dam = 0.25,
        isArm = true,
        skeletal = {"Ulna", "Radius"},
        muscular = {"Palmaris Longus", "Supinator Longus", "Flexor Carpi Radialis"},
        vital = {}
    };
    aCritLocations["Wrist, back"] = {
        dam = 0.25,
        isArm = true,
        skeletal = {"1d8 Carpus Bones"},
        muscular = {"Anterior Annular Ligament of the Wrist"},
        vital = {}
    };
    aCritLocations["Wrist, front"] = {
        dam = 0.25,
        isArm = true,
        skeletal = {"1d8 Carpus Bones"},
        muscular = {"Anterior Annular Ligament of the Wrist"},
    ...
    aCritLocations["Hand, back"] = {
        dam = 0.1,
        isArm = true,
        skeletal = {"1d5 Metacarpus Bones"},
        muscular = {"Extensor Communis Digitorum", "1d5 Flexor Tendons of the Fingers"},
        vital = {}
    };
    aCritLocations["Palm"] = {
        dam = 0.1,
        isArm = true,
        skeletal = {"1d5 Metacarpus Bones"},
        muscular = {"Plamaris Brevis", "Abductor Pollicis"},
        vital = {}
    };
    aCritLocations["Finger(s)"] = {
        dam = 0.01,
        isArm = true,
        isDigit = true,
        skeletal = {"1d4+1 Phalanges"},
        muscular = {"1d5 Flexor Tendons of the Fingers"},
        vital = {}
    };
    aCritLocations["Shoulder, side"] = {
        dam = 0.3,
        isArm = true,
        skeletal = {"Head of the Humerus"},
        muscular = {"Deltoid", "Long Tendon of the Biceps Flexor Cubiti", "Tendon of the Pectoralis Major",
                    "Coraco-brachialis"},
        vital = {}
    };
    aCritLocations["Shoulder, top"] = {
        dam = 0.3,
        isArm = true,
        skeletal = {"Head of the Humerus", "Coracoid Process of the Scapula", "Clavicle"},
        muscular = {"Subclavius", "Deltoid"},
        vital = {}
    };
    aCritLocations["Neck, front"] = {
        dam = 1.0,
        isHead = true,
        isSpine = true,
        skeletal = {"1d4 Cervical Vertebrae from 7th to 4th", "1d4 Cervical Vertebrae from 2nd to 5th"},
        muscular = {"Sterno-hyoid Muscle", "Omo-Hyoid Muscle", "Thyro-hyoid Muscle"},
        vital = {"Trachea", "Spinal Cord"}
    };
    aCritLocations["Neck, back"] = {
        dam = 1.0,
        isHead = true,
        isSpine = true,
        skeletal = {"1d6 Cervical Vertebrae from 2nd to 7th"},
        muscular = {"Trapezius"},
        vital = {"Spinal Cord"}
    };
    aCritLocations["Neck, side"] = {
        dam = 1.0,
        isHead = true,
        isSpine = true,
        skeletal = {"1d4 Cervical Vertebrae from 7th to 4th", "1d4 Cervical Vertebrae from 2nd to 5th"},
        muscular = {"Platysma Moyides", "Scalenus Anticus, Medius and Posticus"},
        vital = {"Trachea", "Spinal Cord"}
    };
    aCritLocations["Head, side"] = {
        dam = 1.0,
        isHead = true,
        skeletal = {"Parietal Bone"},
        muscular = {"Masseter"},
        vital = {"Brain"}
    };
    aCritLocations["Head, back lower"] = {
        dam = 1.0,
        isHead = true,
        isSpine = true,
        skeletal = {"Basilar Process of the Occipital Bone", "First Cervical Vertebrae", "Occipital Bone"},
        muscular = {"Trapezius"},
        vital = {"Medulla Oblongata", "Spinal Cord"}
    };
    aCritLocations["Face, lower side"] = {
        dam = 1.0,
        isHead = true,
        skeletal = {"Inferior Maxillary Bone", "Mastoid Temporal Bone"},
        muscular = {"Zygomaticus Major and Minor", "Buccinator"},
        vital = {}
    };
    aCritLocations["Face, lower center"] = {
        dam = 1.0,
        isHead = true,
        isSpine = true,
        skeletal = {"Inferior Maxillary Bone", "Teeth", "Superior Maxillary Bones"},
        muscular = {"Quadratus Menti", "Quadratus Oris"},
        vital = {"Spinal Cord"}
    };
    aCritLocations["Head, back upper"] = {
        dam = 1.0,
        isHead = true,
        skeletal = {"Parietal Bones"},
        muscular = {"Occipito-frontalis"},
        vital = {"Brain"}
    };
    aCritLocations["Face, upper side"] = {
        dam = 1.0,
        isHead = true,
        skeletal = {"Temporal Bone", "Sphenoid Bone"},
        muscular = {"Temporalis"},
        vital = {"Brain"}
    };
    aCritLocations["Face, upper center"] = {
        dam = 1.0,
        isHead = true,
        skeletal = {"Nasal Bones", "Frontal Bone, Lower", "Frontal Bone, Upper"},
        muscular = {"Occipito-frontalis", "Orbicularis Palpebrarum", "Levator Labii Superioris"},
        vital = {"Brain"}
    };
    aCritLocations["Head, top"] = {
        dam = 1.0,
        isHead = true,
        skeletal = {"Frontal Bone"},
        muscular = {"Occipito-frontalis"},
        vital = {"Brain"}
    };
end
