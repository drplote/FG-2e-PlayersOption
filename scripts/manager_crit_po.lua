...
function handlePlayersOptionCrit(rRoll, rAction, nDefenseVal, rSource, rTarget)
    ...
    local nSizeDifference = getSizeDifference(nodeAttacker, nodeDefender, rWeaponInfo) +
                                getCritSizeBonus(rSource, rTarget);
    ...
    DebugPO.log("Crit with weapon: ", rWeaponInfo, "hit location:", rHitLocation, "size difference", nSizeDifference,
        "severity", nSeverity);
    return rCrit;
end
...
function addCritMessage(rCrit)
    ...
    if not rCrit.error then
        local sDesc = string.format(". %s Save vs death to avoid effects. %s regardless of save result.", rCrit.desc,
            sDmgMultMsg);
...
function getCritResult(rWeaponInfo, nodeDefender, rHitLocation, nSeverity)
    ...
    if not sDamageType or not sDefenderType or not rHitLocation or not rHitLocation.locationCategory or not nSeverity then
        ...
        DebugPO.log("couldn't generate crit", "sDefenderType", sDefenderType, "sDamageType", sDamageType,
            "rHitLocation", rHitLocation, "nSeverity", nSeverity);
        rCrit = {};
        rCrit.error = true;
    else
        local nSeverityIndex = nSeverity;
        if nSeverityIndex > 13 then
            nSeverityIndex = 13;
        end
        if nSeverityIndex < 1 then
            nSeverityIndex = 1;
        end
