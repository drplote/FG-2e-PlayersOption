...
function getBaseActorSpeedPhase(nodeActor)
    ...
    DebugPO.log("Base Speed Phase", nBaseSpeed, "Movement Rate Modifier", nMovementRateModifierToPhase,
        "Encumbrance Modifier", nEncumbranceModifierToPhase);
...
function moveHackmasterActorToNextRound(nodeCT, nInitMod)
    ...
    elseif InitManagerPO.hasAdditionalInitsInQueue(nodeCT) then
        ...
        if (bIsPc and PlayerOptionManager.isDefaultingPcInitTo99()) or
            (not bIsPc and PlayerOptionManager.isDefaultingNpcInitTo99()) then
            nInitResult = 99;
        else
